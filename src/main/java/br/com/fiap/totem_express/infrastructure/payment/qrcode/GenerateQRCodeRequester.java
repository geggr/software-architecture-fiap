...
public class GenerateQRCodeRequester implements QRCodeGateway {
    ...
    @Override
    public QRCodeResponse generateQRCode(Order order) {
        RequestEntity<QRCodeRequest> request = buildRequest(order);

        try {
            final var response = this.restTemplate.exchange(request, QRCodeResponse.class);
    ...
    private RequestEntity<QRCodeRequest> buildRequest(Order order) {
